...
class InvestmentAdviceAgent:
    ...
    def generate_advice_report(self, best_factor: dict) -> str:
        ...
        제시된 알파 팩터를 분석하여, 다음 목차와 세부 가이드에 따라 전문가 수준의 '알파 팩터 분석 리포트'를 Markdown 형식으로 작성해주세요.

        <알파 팩터 분석 리포트>
        1. 팩터 개요 (Factor Overview)
        2. 투자 전략 설계 (Strategy Design)
        """

        user_prompt = f"""
        다음은 새롭게 발굴된 우수한 알파 팩터의 정보입니다. 이 정보를 바탕으로 '알파 팩터 분석 리포트'를 작성해주세요.
